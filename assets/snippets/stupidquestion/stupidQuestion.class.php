...
if (!class_exists('stupidQuestion')) {
	...
	class stupidQuestion {
		...
		function setQuestion() {
			...
			if (!count($_POST)) {
				unset($_SESSION['StupidQuestion'], $_SESSION['StupidQuestionAnswer']);
			}
			...
			if (isset($_SESSION['StupidQuestion'])) {
				foreach ($this->settings['formFields'] as $formKey => $formField) {
					if (in_array($formField, array_keys($_POST)) && $_POST[$formField] != '') {
			...
			$question = $this->settings['questions'][$randQuestion];
